...
export function AccountPanel() {
  ...
  return <section className="card"><p className="eyebrow">ACCOUNT</p><h1>Your FanBackstage</h1><p>{user.email}</p><p>Roles: {user.roles.join(", ")}</p><Link className="button" href="/creator-onboarding">Become a creator</Link>{user.roles.includes("creator") && <Link className="button" href="/creator-studio">Creator studio</Link>}<button onClick={logout}>Log out</button></section>;
}
