...
export function GroupMemberships() {
  ...
  return <section><h2>Group contracts</h2><ul>{items.map((item) => <li key={item.id}>Group {item.group_id}: {item.status} {item.contracts.map((contract) => <span key={contract.id}> v{contract.version}: {contract.creator_basis_points / 100}% / {contract.group_basis_points / 100}% ({contract.status}) {contract.status === "proposed" && contract.version > 1 && <><button onClick={() => act(`/groups/contracts/${contract.id}/accept`)}>Accept amendment</button><button onClick={() => act(`/groups/contracts/${contract.id}/reject`)}>Reject amendment</button></>}</span>)} {item.status === "invited" && <><button onClick={() => act(`/groups/memberships/${item.id}/accept`)}>Accept</button><button onClick={() => act(`/groups/memberships/${item.id}/reject`)}>Reject</button></>} {item.status === "active" && <><button onClick={() => act(`/groups/memberships/${item.id}/leave`)}>Leave</button><label><input type="checkbox" checked={item.affiliation_public} onChange={(e) => act(`/groups/memberships/${item.id}/affiliation`, "PATCH", { visible: e.target.checked })} /> Show affiliation</label></>}</li>)}</ul>{error && <p className="error">{error}</p>}</section>;
}
