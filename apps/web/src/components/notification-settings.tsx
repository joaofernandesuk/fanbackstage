...
export function NotificationSettings() {
  ...
  async function save(category: string, email_enabled: boolean) {
    if (category === "marketing" && !email_enabled) {
      await api("/notifications/unsubscribe", { method: "POST" });
    } else {
      await api(`/notifications/preferences/${category}`, { method: "PUT", body: JSON.stringify({ email_enabled, in_app_enabled: true, consent: category === "marketing" && email_enabled }) });
    }
    await load();
  }
