...
export default function CreatorConsentPage() {
  ...
  return <section className="card"><p className="eyebrow">CREATOR STUDIO</p><h1>Consent releases</h1><p>Releases remain pending until an authorized Trust & Safety reviewer verifies them.</p><label>Creator ID<input value={creatorId} onChange={(e) => setCreatorId(e.target.value)} /></label><button type="button" disabled={!creatorId} onClick={() => void load()}>Load releases</button><label>Linked content ID<input value={contentId} onChange={(e) => setContentId(e.target.value)} /></label><label>Participant reference<input value={participant} onChange={(e) => setParticipant(e.target.value)} /></label><button type="button" disabled={!creatorId || !contentId || !participant} onClick={() => void submit()}>Submit release</button><ul>{releases.map((release) => <li key={release.id}>{release.release_type} · {release.status}{release.effective_until ? ` · expires ${new Date(release.effective_until).toLocaleString()}` : ""}{release.status === "verified" && <button type="button" aria-label={`Revoke release ${release.id}`} onClick={() => void revoke(release.id)}>Revoke</button>}</li>)}</ul>{error && <p className="error">{error}</p>}</section>;
}
