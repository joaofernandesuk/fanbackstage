...
test("Phase 13 real-stack report, appeal, consent, and critical-containment journeys", async ({ page }) => {
  ...
  await logout(page);
  await login(page, creator.email, creator.password);
  const booking = await apiOk<{ id: string }>(page, "/featuring/bookings", "POST", { slot_id: slot.id, target_type: "gallery", target_id: reportable.id, starts_at: new Date(Date.now() + 5_000).toISOString(), duration_seconds: 120 }, `phase13-booking-${stamp}`);
  ...
  await page.getByRole("button", { name: "Apply to case target" }).click();
  ...
  await page.getByRole("button", { name: new RegExp(reportable.title) }).click();
  await page.getByLabel("Why should this decision be reviewed?").fill("The report does not support the action.");
  ...
  await page.getByRole("button", { name: new RegExp(reportable.title) }).click();
  await page.locator('label:has-text("Outcome") select').selectOption("overturned");
  await page.getByLabel("Decision reason").fill("Independent reviewer overturned containment.");
  await page.getByRole("checkbox", { name: /confirm this outcome/ }).check();
  ...
  const performer = await api<{ id: string }>(page, "/performers", "POST", { safe_reference: "private-performer-reference", country_code: "PT" });
  expect(performer.status).toBe(201);
  await page.goto("/creator-studio/consent");
  await page.getByLabel("Content").selectOption({ label: `${consented.title} · gallery` });
  await page.getByLabel("Participant / performer").selectOption({ label: "private-performer-reference · PT" });
  ...
  await page.getByRole("button", { name: / private-performer-reference$/ }).click();
  await page.getByLabel("Reason").fill("Consent scope and evidence are valid.");
  await page.getByRole("checkbox", { name: /confirm this review outcome/ }).check();
  await page.getByRole("button", { name: "Record decision" }).click();
  await expect(page.getByRole("status")).toContainText("recorded");
  await page.getByRole("button", { name: /separate-private-performer-reference/ }).click();
  await expect(page.getByRole("heading", { name: "separate-private-performer-reference", exact: true })).toBeVisible();
  await page.getByLabel("Reason").fill("Separate consent scope is valid.");
  await page.getByRole("checkbox", { name: /confirm this review outcome/ }).check();
  await page.getByRole("button", { name: "Record decision" }).click();
  ...
  await logout(page);
  await login(page, creator.email, creator.password);
  const consentBooking = await apiOk<{ id: string }>(page, "/featuring/bookings", "POST", { slot_id: slot.id, target_type: "gallery", target_id: consented.id, starts_at: new Date(Date.now() + 5_000).toISOString(), duration_seconds: 120 }, `phase13-consent-booking-${stamp}`);
  ...
  await page.getByRole("button", { name: "Revoke release for private-performer-reference" }).click();
  ...
  await page.goto("/moderation/consent");
  await page.getByRole("button", { name: /private-performer-reference/ }).click();
  await page.getByLabel("Reason").fill("Replacement release evidence is valid.");
  await page.getByRole("checkbox", { name: /confirm this review outcome/ }).check();
  await page.getByRole("button", { name: "Record decision" }).click();
  await expect.poll(async () => (await api(page, `/content/public/${consented.id}`)).status).toBe(200);
  expect((await api(page, `/content/public/${separatelyConsented.id}`)).status).toBe(200);
  ...
  await login(page, creator.email, creator.password);
  await page.goto("/appeals");
  await page.getByRole("button", { name: new RegExp(critical.title) }).click();
  await page.getByLabel("Why should this decision be reviewed?").fill("Containment can be safely overturned after review.");
  await page.getByRole("button", { name: "Submit appeal" }).click();
  await logout(page);
  await login(page, reviewer.email, reviewer.password);
  await page.goto("/moderation/appeals");
