...
test("Phase 6 messaging uses the real payment, inbox, campaign, and moderation stack", async ({ browser, page }) => {
  ...
  await login(page, creatorEmail, password); expect((await api(page, "/messages/settings", "PUT", { permission: "anyone", send_fee_minor: 250, send_fee_currency: "EUR", subscribers_free: false })).status).toBe(200); await page.getByRole("button", { name: "Log out" }).click(); await login(viewer, viewerEmail, password);
