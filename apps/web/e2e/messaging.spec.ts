...
test("Phase 6 messaging uses the real payment, inbox, campaign, and moderation stack", async ({ browser, page }) => {
  ...
  await register(page, creatorEmail, password); await page.getByRole("link", { name: "Become a creator" }).click(); await page.getByRole("textbox", { name: /^Your @handle/ }).fill(username); await page.getByLabel("Display name").fill("Messaging creator"); await page.getByRole("button", { name: "Save profile" }).click(); await page.getByRole("button", { name: "Submit application" }).click(); await page.getByRole("button", { name: /^(Complete development verification|Complete identity check)$/ }).click(); await logout(page);
