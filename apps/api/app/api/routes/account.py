...
from app.api.deps import Db, RawCurrentIdentity
...
async def me(identity: RawCurrentIdentity) -> UserResponse:
    user, _ = identity
...
async def sessions(identity: RawCurrentIdentity, db: Db) -> list[SessionResponse]:
...
async def delete_session(
    session_id: str, identity: RawCurrentIdentity, db: Db, request: Request
) -> MessageResponse:
    user, _ = identity
...
async def delete_other_sessions(
    identity: RawCurrentIdentity, db: Db, request: Request
) -> MessageResponse:
    user, current = identity
