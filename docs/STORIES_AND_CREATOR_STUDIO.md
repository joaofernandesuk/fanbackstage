...
The current photo composer renders a new flattened composition in the creator's browser and sends it through the existing protected asset-processing pipeline. It now includes an explicit, opt-in camera-photo path with local face-landmark effects: lip colour, neon eyes, and a star mask. The browser requests camera permission only after the creator activates it. The vendored face-landmark runtime and model run locally; camera frames and landmark coordinates are neither persisted nor sent to a third party. Only the creator's captured, flattened photo is uploaded when they later choose Publish. Effects have an ordinary photo-upload fallback if camera or landmark support is unavailable. Camera video recording, video face effects, and a non-destructive server-side editor remain future work.
